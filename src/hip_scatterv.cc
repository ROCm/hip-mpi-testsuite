...
int scatterv_test(void *sendbuf, void *recvbuf, int count,
...
int main(int argc, char *argv[])
{
    ...
    // Warmup
    res = scatterv_test(sendbuf->get_buffer(), recvbuf->get_buffer(), elements,
                        MPI_DOUBLE, MPI_COMM_WORLD, 1);
    if (MPI_SUCCESS != res)
    {
        fprintf(stderr, "Error in scatterv_test. Aborting\n");
        MPI_Abort(MPI_COMM_WORLD, 1);
        return 1;
    }

    // execute the scatterv test
